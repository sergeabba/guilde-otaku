...
export default function FightersPage() {
  ...
  const StatBar = ({ label, value, color }: { label: string, value: number, color: string }) => (
    <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: isMobile ? "2px" : "4px", width: "100%" }}>
      ...
      <span style={{ flexShrink: 0, fontSize: isMobile ? "12px" : "16px", fontWeight: 900, width: isMobile ? "20px" : "26px", textAlign: "right", color: "#fff", textShadow: "0 2px 4px #000" }}>{value}</span>
    </div>
  );
  ...
  return (
    <div style={{ height: "100vh", width: "100vw", display: "flex", flexDirection: "column", background: "#050505", overflow: "hidden", fontFamily: "'Barlow Condensed', sans-serif", color: "#fff", position: "relative" }}>
      ...
      <style>{`
        ...
        .smooth-scroll::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.2); border-radius: 10px; }
        .smooth-scroll::-webkit-scrollbar-thumb:hover { background: rgba(255,255,255,0.4); }
      `}</style>

      {/* BACKGROUND ARENA */}
      <AnimatePresence mode="wait">
        <motion.div key={currentArena.id} initial={{ opacity: 0, scale: 1.02 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.8 }}
          style={{ position: "absolute", inset: 0, backgroundImage: `url(${currentArena.bg})`, backgroundSize: "cover", backgroundPosition: "center", filter: "brightness(0.3) contrast(1.1)", zIndex: 0 }}
        />
      </AnimatePresence>
      <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at center, transparent 10%, rgba(0,0,0,0.9) 100%)", zIndex: 1, pointerEvents: "none" }} />

      {/* HEADER NAVBAR */}
      <header style={{ position: "relative", zIndex: 30, display: "flex", alignItems: "center", justifyContent: "space-between", padding: isMobile ? "10px 15px" : "20px 40px", background: "linear-gradient(to bottom, rgba(0,0,0,0.9), transparent)", flexShrink: 0 }}>
        <Link href="/" style={{ color: "#fff", textDecoration: "none", fontSize: isMobile ? "14px" : "18px", fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.1em", textShadow: "0 2px 4px #000" }}>← RETOUR</Link>
        <div style={{ display: "flex", alignItems: "center", gap: isMobile ? "10px" : "20px" }}>
          <div style={{ display: "flex", background: "rgba(0,0,0,0.6)", borderRadius: "6px", padding: "4px", border: "1px solid rgba(255,255,255,0.1)", backdropFilter: "blur(10px)" }}>
            {(["real", "anime"] as ViewMode[]).map((mode) => (
              <button key={mode} onClick={() => setViewMode(mode)} style={{ fontSize: isMobile ? "10px" : "12px", fontWeight: 900, textTransform: "uppercase", padding: isMobile ? "6px 12px" : "8px 16px", borderRadius: "4px", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px", background: viewMode === mode ? "#f03e3e" : "transparent", color: viewMode === mode ? "#fff" : "rgba(255,255,255,0.4)", transition: "0.3s" }}>
                {mode === "real" ? <User size={12} /> : <Sword size={12} />} {!isMobile && mode}
              </button>
            ))}
          </div>
          <div style={{ display: "flex", gap: "10px" }}>
            <button onClick={toggleSound} style={{ background: "transparent", border: "none", cursor: "pointer", color: isPlaying ? "#f03e3e" : "#fff", filter: "drop-shadow(0 2px 4px #000)" }}>{isPlaying ? <Volume2 size={20} /> : <VolumeX size={20} />}</button>
            <button onClick={nextTrack} style={{ background: "transparent", border: "none", cursor: "pointer", color: "#fff", filter: "drop-shadow(0 2px 4px #000)" }}><SkipForward size={20} /></button>
          </div>
        </div>
      </header>
      ...
      <div style={{ textAlign: "center", position: "relative", zIndex: 20, padding: isMobile ? "5px 0" : "0 0 10px 0", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "15px", marginTop: "2px" }}>
          ...
          <div style={{ textAlign: "center", minWidth: "150px" }}>
            ...
            <p style={{ fontSize: "clamp(12px, 2vw, 16px)", fontWeight: 900, color: currentArena.color, textTransform: "uppercase", margin: 0, textShadow: `0 0 15px ${currentArena.color}80` }}>{currentArena.name}</p>
          </div>
      ...
      {isMobile ? (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", position: "relative", zIndex: 10, overflow: "hidden" }}>
          ...
          <div style={{ flex: "0 0 48%", display: "flex", borderBottom: "2px solid rgba(255,255,255,0.1)", background: "rgba(0,0,0,0.5)" }}>
            ...
            {/* VS LOGO MOBILE (PROPREMENT CENTRÉ) */}
            {p1 && p2 && (
              <div style={{ position: "absolute", left: "50%", top: "50%", transform: "translate(-50%, -50%)", zIndex: 30, pointerEvents: "none" }}>
                <div style={{ fontSize: "32px", fontWeight: 900, fontStyle: "italic", background: "linear-gradient(to bottom, #fff, #aaa)", WebkitBackgroundClip: "text", color: "transparent", WebkitTextStroke: "1px #f03e3e", filter: "drop-shadow(0 4px 10px #000)" }}>V.S.</div>
              </div>
            )}
          </div>

          {/* BAS : GRILLE DE SÉLECTION MOBILE */}
          <div className="smooth-scroll" style={{ flex: 1, overflowY: "auto", padding: "15px", background: "rgba(0,0,0,0.6)", backdropFilter: "blur(15px)" }}>
              ...
              {members.map((m) => {
                ...
                return (
                  <button key={m.id} onClick={() => handleSelect(m)} style={{
                      aspectRatio: "1/1", width: "100%", padding: 0, background: "#111", cursor: "pointer", position: "relative",
                      borderRadius: "12px", border: isP1 ? "2px solid #f03e3e" : isP2 ? "2px solid #3b82f6" : "1px solid rgba(255,255,255,0.15)",
            ...
            <AnimatePresence mode="wait">
              {p1 ? (
                <motion.div key="p1" initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4 }} style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", position: "relative" }}>
                  ...
                  {/* HUD P1 AVEC DEGRADÉ */}
                  <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(0,0,0,1) 5%, rgba(0,0,0,0.6) 40%, transparent 100%)" }} />
                  <div style={{ position: "relative", zIndex: 10, flex: 1, display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: "40px 50px" }}>
                    ...
                    {p1.badge && (
                      <div style={{ display: "inline-flex", alignSelf: "flex-start", background: "linear-gradient(135deg, #b8860b, #ffd700, #b8860b)", padding: "4px 10px", borderRadius: "6px", alignItems: "center", gap: "6px", marginBottom: "10px", boxShadow: "0 4px 15px #000" }}>
                        ...
                        <span style={{ fontSize: "12px", fontWeight: 900, color: "#000", textTransform: "uppercase", letterSpacing: "0.05em" }}>{p1.badge}</span>
                      </div>
                    )}
                    ...
                    <div style={{ background: "rgba(240,62,62,0.15)", borderLeft: "4px solid #f03e3e", padding: "15px", borderRadius: "0 12px 12px 0", backdropFilter: "blur(10px)", maxWidth: "450px" }}>
                ...
                <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(240,62,62,0.4)", fontSize: "60px", fontWeight: 900, fontStyle: "italic", letterSpacing: "0.15em" }}>P1 SELECT</div>
              )}
            </AnimatePresence>
          </div>

          {/* --- GRILLE CENTRALE DESKTOP --- */}
          <div style={{ width: "550px", height: "100%", display: "flex", flexDirection: "column", background: "rgba(0,0,0,0.5)", backdropFilter: "blur(20px)", borderLeft: "1px solid rgba(255,255,255,0.08)", borderRight: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 0 60px #000", zIndex: 30 }}>
            <div style={{ padding: "20px", textAlign: "center", borderBottom: "1px solid rgba(255,255,255,0.05)", background: "linear-gradient(to bottom, rgba(0,0,0,0.6), transparent)" }}>
            ...
            <div className="smooth-scroll" style={{ flex: 1, overflowY: "auto", padding: "25px", display: "grid", justifyContent: "center", gridTemplateColumns: "repeat(auto-fill, minmax(80px, 1fr))", gap: "20px", alignContent: "start" }}>
              {members.map((m) => {
                const isP1 = p1?.id === m.id;
                const isP2 = p2?.id === m.id;
                const isSelected = isP1 || isP2;
                ...
                return (
                  <motion.button key={m.id} whileHover={{ scale: 1.1, zIndex: 20, boxShadow: "0 10px 25px rgba(0,0,0,0.8)" }} whileTap={{ scale: 0.95 }} onClick={() => handleSelect(m)} style={{
                      aspectRatio: "1/1", width: "100%", padding: 0, background: "rgba(255,255,255,0.02)", cursor: "pointer", position: "relative",
                      borderRadius: "14px", border: isP1 ? "3px solid #f03e3e" : isP2 ? "3px solid #3b82f6" : "2px solid rgba(255,255,255,0.15)",
                      overflow: "hidden", opacity: isDimmed ? 0.3 : 1, filter: isDimmed ? "grayscale(100%)" : "none",
                    ...
                    <img src={viewMode === "anime" ? m.animeChar : m.photo} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "top" }} alt={m.name} />
                    {m.badge && <div style={{ position: "absolute", top: "6px", right: "6px", background: "linear-gradient(135deg, #b8860b, #ffd700)", borderRadius: "50%", padding: "4px", boxShadow: "0 2px 8px #000" }}><Trophy size={12} color="#000" /></div>}
              ...
              {p2 ? (
                <motion.div key="p2" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 50 }} transition={{ duration: 0.4 }} style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", position: "relative" }}>
                  ...
                  <div style={{ position: "relative", zIndex: 10, flex: 1, display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: "40px 50px", alignItems: "flex-end", textAlign: "right" }}>
                    ...
                    {p2.badge && (
                      <div style={{ display: "inline-flex", alignSelf: "flex-end", background: "linear-gradient(135deg, #b8860b, #ffd700, #b8860b)", padding: "4px 10px", borderRadius: "6px", alignItems: "center", gap: "6px", marginBottom: "10px", boxShadow: "0 4px 15px #000" }}>
                        <span style={{ fontSize: "12px", fontWeight: 900, color: "#000", textTransform: "uppercase", letterSpacing: "0.05em" }}>{p2.badge}</span>
                        <Trophy size={14} color="#000" />
                      </div>
                    )}
                    ...
                    <div style={{ background: "rgba(59,130,246,0.15)", borderRight: "4px solid #3b82f6", padding: "15px", borderRadius: "12px 0 0 12px", backdropFilter: "blur(10px)", maxWidth: "450px", display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
                ...
                <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(59,130,246,0.4)", fontSize: "60px", fontWeight: 900, fontStyle: "italic", letterSpacing: "0.15em" }}>P2 SELECT</div>
              )}
            </AnimatePresence>
          </div>
